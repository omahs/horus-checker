...
(assert (= ADDR!10 (mod (+ ap!<root>@5 21) prime)))
(assert (= ADDR!11 (mod (+ fp!<49=read/root> (- 4)) prime)))
(assert (= ADDR!12 (mod (+ ap!<root>@5 20) prime)))
(assert (= ADDR!13 (mod (+ ap!<root>@5 22) prime)))
(assert (= ADDR!14 (mod (+ fp!<49=read/root> (- 3)) prime)))
...
(assert (< (mod (+ ap!<root>@5 3) prime) (mod (+ ap!<root>@5 23) prime)))
(assert (and (<= pedersen!start MEM!5) (<= MEM!10 pedersen!end)))
(assert true)
(assert true)
(assert true)
(assert (and (<= range-check!start MEM!7) (<= MEM!13 range-check!end)))
...
(assert (not (and (<= pedersen!start MEM!11) (<= MEM!12 pedersen!end) (<= range-check!start MEM!14) (<= MEM!10 range-check!end))))
(check-sat)